...
- サブエージェントの Discord スレッド可視化: エージェントがサブターンを生成するたびに Discord スレッドを開き、中間イベント（ツール実行の開始/完了（名前・引数・所要時間）、LLM 応答（統計情報）、エラー）をリアルタイムで投稿します。スレッド作成ができない場合（DM 等）は安全にフォールバックします。
...
| `0004-add-subagent-discord-visibility.patch` | `pkg/channels/discord/discord.go`, `pkg/channels/interfaces.go`, `pkg/gateway/gateway.go` を修正、`pkg/gateway/subturn_notifier.go` を新規追加 | サブターンイベントごとに Discord スレッドを開き、中間イベント（ツール実行の開始/完了（名前・引数プレビュー・所要時間・エラー状態）、LLM 応答（文字数・ツール呼び出し数・推論フラグ）、エラー）をリアルタイムで投稿する。2 ゴルーチン構成（EventBus 購読の `eventLoop` で状態管理、Discord I/O 用 `workerLoop`）で gateway をブロックしない。`ThreadCapable` チャンネルインターフェースを導入し、Discord メンション混入を防ぐサニタイズを実施、スレッド作成不能時（DM 等）は安全にフォールバック。 |
