...
- Subagent Discord thread visibility: when an agent spawns a subturn, a Discord thread is opened so users can follow subagent activity in real time. Intermediate events — tool execution start/end (with name, arguments, duration), LLM responses (with stats), and errors — are posted to the thread as they happen. Falls back gracefully when thread creation is unavailable (e.g. DMs).
...
| `0004-add-subagent-discord-visibility.patch` | Modifies `pkg/channels/discord/discord.go`, `pkg/channels/interfaces.go`, `pkg/gateway/gateway.go`, and adds `pkg/gateway/subturn_notifier.go` | Opens a Discord thread per subturn event and posts intermediate progress — tool execution start/end (name, arguments preview, duration, error status), LLM responses (content length, tool call count, reasoning flag), and errors — so users can observe subagent activity in real time. Uses a two-goroutine architecture (EventBus `eventLoop` for state tracking, `workerLoop` for Discord I/O) to avoid blocking the gateway. Introduces a `ThreadCapable` channel interface, sanitizes Discord mentions in user-facing text, and falls back gracefully when thread creation isn't possible (e.g. DMs). |
